# RFC: Proposal for a Coordinated 2 m Channel for Permanent Unmanned Amateur Winlink Stations in IARU Region 1
...
This document proposes the coordinated use of a single dedicated 2 m amateur-radio frequency — **144.850 MHz** — for **permanent, unmanned amateur stations that provide Winlink functionality** across IARU Region 1 (Europe). It is deliberately written as a **neutral framework for fixed unmanned Winlink stations** — not around any one organisation, software package, or emergency-communication service. The **Dutch Winlink System (DWS)** is used as one running example throughout — it is the current deployment on 144.850 MHz — but it is only **one** implementation. The Winlink ecosystem also includes fully **open-source** components: the cross-platform **Pat** client, the open **ARDOP** modem (e.g. the MIT-licensed `ardopcf`), and open **AX.25** packet. The proposal is meant to be equally usable by any association, country, or station-keeper in Region 1, whatever software they run (see also the open-source direction in [Section 14](#14-software-openness-and-a-path-to-an-open-modem)). It began as a Belgian proposal (a coordination request from the RST club in Sint-Truiden to the UBA unmanned-station coordination, around the case of ON0BAF) and has been broadened to a Region-1 scope, because cross-border digital messaging is inherently border-crossing: a channel that stops at a national boundary cannot serve a European Winlink network.

The goal is to obtain broad technical and organisational backing — from national societies and, ultimately, the IARU Region 1 VHF/UHF/Microwaves Committee (C5) — for **harmonising the unmanned Winlink 2 m channel on the frequency that has already become the de-facto standard in the Netherlands and that is already in coordinated use in Belgium**, so that the network remains compatible with the existing 2 m band plan and so that a single channel can be used by amateurs on both sides of a border without re-tuning.

This is not a request to invent a new mode or a new band segment. Unlike a spread-spectrum experiment, an unmanned Winlink station transmits an **ordinary narrow-band FM signal** (audio from the Winlink software fed into a standard FM transceiver — see [Section 3](#3-how-winlink-vara-fm-and-dws-work-on-the-air)); it already belongs in the 2 m digital-communications segment. What is missing is *coordination*: agreeing on one common frequency (as APRS long ago agreed on 144.800 MHz), and a **simplified unmanned-station coordination procedure** for these stations, analogous to the one that already exists for APRS (see [Section 10](#10-licensing-and-unmanned-station-status) and [Section 13](#13-coordination-proposal)).

### 1.1 Scope: permanent, coordinated unmanned stations only

To keep the proposal sharp and neutral, its scope is drawn deliberately narrow.

- **In scope.** This RFC concerns **permanent, registered and coordinated unmanned amateur Winlink stations** — fixed installations whose location, antenna height, power, coverage, on-air mode(s) and spacing from other stations are known in advance, and which are brought on the air through a national coordination and registration process.
- **Out of scope.** This RFC does **not** concern ad-hoc deployment, temporary field setups, emergency deployment without prior coordination, or portable stations built up locally during an incident. Those remain a matter of on-the-day operating practice, not band-plan coordination.

**Why the distinction matters.** The usual objections to a shared channel — congestion, too little spacing between stations, and practical deployability — are really objections to **ad-hoc** use, where conditions vary, coordination is hard, and a local frequency choice may be unavoidable. For **permanent** unmanned stations the situation is the opposite: because siting, power, coverage and mode are fixed and known beforehand, frequency use can be **coordinated in advance** and overlap deliberately bounded (the concrete techniques for making stations overlap *without* disturbing each other are set out in [Section 11.2](#112-coordinating-co-channel-stations--overlapping-coverage-without-mutual-interference)). So the real dividing line is not *"everything on one frequency"* versus *"every station its own frequency"* — it is **ad-hoc versus permanent, coordinated** operation. This RFC is exclusively about the latter.
...
The proposed system is a European (IARU Region 1) amateur **Winlink** digital-messaging layer on the 2 m band, built from **permanent, coordinated unmanned Winlink gateway (and optional digipeater) stations** sharing one coordinated FM channel. (DWS is the current implementation of such stations — [Section 1.1](#11-scope-permanent-coordinated-unmanned-stations-only).)
...
- an ad-hoc, temporary or uncoordinated deployment — this RFC covers **permanent, coordinated** stations only ([Section 1.1](#11-scope-permanent-coordinated-unmanned-stations-only)).
...
### 11.2 Coordinating co-channel stations — overlapping coverage without mutual interference

Where [Section 11.1](#111-co-siting-at-existing-sites) deals with two stations on **one mast**, this section deals with the **network**: many permanent stations sharing the **one** channel. On a shared channel, stations are *meant* to overlap — overlap gives redundancy, one-hop relay reach, and a fuller RADAR map — but that overlap must be **engineered in advance, not accidental** (this is the whole point of the permanent, coordinated scope in [Section 1.1](#11-scope-permanent-coordinated-unmanned-stations-only)). Coordinated overlap yields a usable network; uncoordinated overlap yields mutual desense and stations transmitting over each other.

What makes bounded overlap work is the FM **capture effect**: an FM receiver locks to the *stronger* co-channel signal and suppresses the weaker, so at each point in an overlap zone there is a single "winner" rather than a guaranteed collision. The design target, taken from long-standing APRS coordination practice, is to engineer each overlap seam so **one station dominates by about 10 dB** at any given receiver — *not* merely "a few dB." Where two signals are within a few dB of each other they still collide (flutter / picket-fencing), so a near-equal seam is an effective dead zone to be avoided, not relied on.

One caveat raises the stakes here relative to APRS: a **VARA FM connection holds the channel for the whole session** and cannot tolerate a mid-session collision (its busy check happens at *session start*, then it transmits to the end), whereas APRS traffic is short, bursty and loss-tolerant. So on this channel, mutual audibility and level planning matter *more*, not less. The levers below are listed roughly **most-effective first**.

1. **Siting for mutual audibility and capture dominance — the foundation.** Site and level the coordinated stations so they can hear one another (or a common, well-placed station everyone hears), and so one station dominates each overlap seam by ~10 dB. This is the primary defence against the **hidden-transmitter problem**: two stations that cannot hear each other but both reach a common receiver will key at the same time and *both* transmissions are lost — and **no** busy-detect scheme (carrier sense, or VARA FM's session-start detector) can prevent it, because half-duplex FM has no collision detection. Note that the textbook cure for hidden nodes, an RTS/CTS handshake, is **not** part of AX.25 or VARA FM — so on this channel the real defence is mutual-audibility siting, not a protocol handshake.

2. **Minimum ERP, plus antenna height / pattern / downtilt, to bound each footprint.** Run the **lowest ERP that fills the intended cell**, not maximum power (ERP = TX power + antenna gain − feedline and filter losses; document it — [Section 9.2](#92-erp--eirp-consideration)). But power alone is a **blunt** tool: over VHF terrain, path loss is only ~35–40 dB per decade, so it takes roughly a **12 dB** power cut just to halve range, and the coverage edge is gradual. Antenna **height, vertical pattern and downtilt** shape *where* the energy goes and usually bound overlap more sharply — a lower or non-omni antenna plus terrain shadowing gives a cleaner cell edge, and downtilt can steer a null toward a co-channel neighbour. As APRS practice bluntly notes, *"the best base-station antenna is often not a high omni"*: a too-high omni hears and reaches too much and can degrade local reliability. Treat ERP and antenna geometry as **co-primary**, set together to reach the ~10 dB dominance above.

3. **Squelch / data-carrier-detect (DCD) calibration.** Run the receiver with **audio squelch open** and key the busy decision off the **modem's DCD / energy detector** (an audio squelch gate adds attack/decay delay and clips the start of packets). Calibrate empirically: set the RX level / DCD so it *just* asserts on the **weakest station the gateway can copy** — err toward deferring to a low-level signal rather than transmitting over it. Both errors bite: set too sensitive (near the noise floor) and the station defers needlessly on distant signals that would not actually collide, collapsing throughput; set too "deaf" and it keys over transmissions it hears only weakly, destroying both. Because VARA FM only checks the channel at **session start**, its busy detector must reliably trip on a neighbour's signal — which, again, depends on mutual audibility and freedom from desense. *(VARA FM's exact busy-detect behaviour is taken from its documentation and user reports, not a formal spec — treat these as operating guidance.)*

4. **Receiver-side gain management.** A strong nearby station can drive a gateway's front end into compression — **blocking / desensitisation** — cutting its sensitivity to the weak clients it is meant to serve; one dominant neighbour can effectively paralyse a receiver (co-located 2 m stations routinely show 10–15 dB of desense when a neighbour keys). On 2 m, keep any **preamp off** except for genuine weak-signal work (a preamp worsens strong-signal overload), and **pad the front end** with an attenuator to keep it linear; where a gateway is swamped, deliberate RX attenuation can also **shrink its receive footprint** back to its intended cell. One hard limit to state plainly: attenuation cures *overload / desense* (it restores the ability to hear *other* signals) but it does **not** improve a true **co-channel** signal-to-interference ratio — it cuts wanted and unwanted equally. In-band conflict is settled by capture, siting and ERP/footprint, never by attenuation.

5. **Beacon and timing discipline.** Keep the RADAR discovery beacons short, **stagger** their timing and keep the duty cycle low so beacons from different stations do not pile up (DWS already sequences its *own* beacons — [Section 3.3](#33-why-dws-is-standardising-on-vara-fm)). Where mutual audibility genuinely cannot be achieved between two stations that must both be on the channel, a **time-slot discipline** (each transmits in its own window) sidesteps carrier sense altogether.

6. **Coverage-cell planning with bounded overlap.** Plan the fixed stations as cells that **tile with controlled overlap** — overlapping-but-dominant seams (a clear ~10 dB winner at every point) for redundancy and one-hop relay — while **capping how many stations share one RF domain**, because over-dense same-channel siting adds collisions, not coverage. For a coverage gap, prefer a **low-power fill-in** station; for a station that overshoots, **lower its power/height or pad its RX** first. The coordinator assigns each station its cell, ERP, height, azimuth and expected dominant seams, so overlap is intentional and documented.

**The APRS precedent — and its limits.** All of this has a working precedent one channel down: APRS runs as a single-frequency simplex network (144.800 MHz in Region 1) in which overlapping co-channel digipeaters cover whole countries, coordinated by hop count, power (low-power fill-in vs full-service) and placement/capture (~10 dB dominance, generic digipeaters spaced tens of kilometres apart). It proves that coordinated overlapping unmanned stations *can* share one frequency. But the analogy must not be over-drawn: APRS traffic is short, connectionless and loss-tolerant, and its digipeaters store-and-forward — they never key at the same instant, so coverage is built up *serially in time*. A connected VARA FM session is the opposite. So APRS's **quantitative** figures (per-channel station capacity, hop counts, spacing) do **not** transfer and must be re-derived empirically for connected-mode data; what transfers is only the **principle** — FM capture plus coordinated power, placement and siting make single-frequency co-channel coverage work. It is also why the channel is treated as a **calling / coordination channel** ([Section 3.4](#34-radar-the-calling-channel-and-digipeating-as-a-value-add)) — short discovery beacons keep the offered load low, so that the longer data sessions have room.

**Measure the load, adjust, re-measure.** Coordination is a loop, not a one-time plan. Casual listening is unreliable — a badly congested channel can sound almost like a quiet one — so judge it from real **channel-occupancy and retry/collision statistics** (session logs, or known-rate probe traffic), not by ear. If a gap appears, add a low-power fill-in; if a station is causing desense or collisions, lower its power/height or pad its receiver before anything else; and cap the total offered load and station count per RF domain. Anchoring cell-edge and busy thresholds to a known sensitivity reference (e.g. the conventional 12 dB SINAD point, ≈ −119 dBm / 0.2 µV on a modern 2 m receiver) lets the coordinator write concrete numbers instead of per-operator guesswork.

> **Coordinator's checklist (most effective first).**
>
> 1. Site and level stations for **mutual audibility** and **~10 dB capture dominance** at every overlap seam.
> 2. Run the **minimum ERP** that fills the assigned cell; shape the footprint with antenna **height, pattern and downtilt**; document ERP.
> 3. Calibrate **DCD/squelch** to assert on the weakest copyable station; squelch open, keyed off the modem; verify VARA FM's session-start busy detection.
> 4. Manage **RX gain**: preamp off; pad the front end to cure desense and to rein in an over-reaching footprint.
> 5. Keep **beacons** short, staggered and low-duty; use **time-slotting** where audibility cannot be assured.
> 6. Plan **cells with bounded overlap**: fill-in for gaps, reduce power/height for overshoot; cap stations per RF domain.
> 7. **Measure** occupancy and retries; **iterate**.
...
4. **Ask IARU R1 and national societies to recognise/coordinate 144.850 MHz** as the 2 m channel for permanent unmanned Winlink stations (currently VARA FM) — the single change that turns a de-facto convention into a coordinated channel — and to introduce a **simplified unmanned-station coordination procedure** for these stations, modelled on APRS ([Section 10.1](#101-a-simplified-coordination-procedure--analogous-to-aprs)).
5. Maintain a per-country list of known RMS gateways / digipeaters and responsible operators.
6. Start with modest-power deployments and monitor for interference, applying the co-channel coordination techniques in [Section 11.2](#112-coordinating-co-channel-stations--overlapping-coverage-without-mutual-interference).
...
**This RFC states a clear direction: open source is the preferred basis for a coordinated, resilient network.** VARA FM may remain supported in practice for now — as a usable interim solution, for as long as no equivalent open alternative exists — but it should not be treated as the endpoint. The proposal above adopts VARA FM as the on-air data mode because it is what the current implementation (DWS) uses and what already works on 2 m; that is a deliberate, pragmatic, short-term choice. But it carries a strategic dependency a resilient network should weigh openly: **VARA FM is proprietary, closed-source software with a per-user licence model, its usual client (Winlink Express) is Windows-only, and its continuity rests on a single developer.** For a network whose whole purpose is resilience, a **structural dependency on closed binaries, per-user licences, one operating system, or the continuity of a single developer** is itself a point of fragility.

The direction should therefore be explicit: **no structural dependency on closed software**, and a path toward **open protocols, open implementations, and interoperable software.** This section records that direction and proposes a **two-track approach**: keep deploying what is practically available today (VARA FM), while building an open, auditable modem for the longer term. The frequency-coordination request in this RFC (144.850 MHz) is deliberately **separable from the modem question** — the coordinated channel is an ordinary FM channel, and the modem that rides on it can evolve without re-opening the frequency coordination.
...
So the client and TNC layers can already be fully open and cross-platform — and open **modems** exist too: the **ARDOP** protocol (e.g. the MIT-licensed `ardopcf`, which works with Winlink and Pat) is fully open. The gap is specific: there is no open, **VARA-FM-class VHF FM** modem yet, because ARDOP and Mercury are both HF-oriented (see [Section 14.2](#142-mercury--an-open-modem-and-a-possible-fork-base)).
...
14. Is limiting this RFC to **permanent, registered and coordinated unmanned stations** — excluding ad-hoc, portable, and emergency-without-coordination use — the right scope ([Section 1.1](#11-scope-permanent-coordinated-unmanned-stations-only))?
...
- **Scope: permanent, registered and coordinated unmanned Winlink stations only** — not ad-hoc, portable, or emergency-without-coordination deployments ([Section 1.1](#11-scope-permanent-coordinated-unmanned-stations-only))
- **144.850 MHz** as the harmonised Region-1 2 m channel for those permanent unmanned Winlink stations, on a **12.5 kHz FM** raster
...
- **Open source as the preferred direction:** VARA FM used as a **usable interim** solution, not an endpoint; with a parallel open-modem track — the **MercuryFM** fork of Mercury / HERMES ([Section 14.5](#145-mercuryfm--a-call-for-co-developers-and-testers)), plus open tooling such as Pat — pursued so the network takes on **no structural dependency** on closed binaries, per-user licences, Windows-only tooling, or a single developer, and moves toward open, interoperable software (see [Section 14](#14-software-openness-and-a-path-to-an-open-modem))
...
- **ARDOP — Amateur Radio Digital Open Protocol; `ardopcf` open (MIT) implementation** — open sound-card modem usable with Winlink and Pat (HF-oriented)
